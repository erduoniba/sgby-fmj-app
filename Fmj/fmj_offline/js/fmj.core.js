...
this['fmj.core'] = function (_, Kotlin) {
  ...
  MainGame.prototype.listenUIEvents_0 = function (multiple) {
    var delta = L40;
    sysAddKeyDownListener(MainGame$listenUIEvents$lambda(this));
    sysAddKeyUpListener(MainGame$listenUIEvents$lambda_0(this));
    let gameSpeedMultiple = 1;
    if (multiple > 0.2 && multiple < 5)  {
        gameSpeedMultiple = multiple;
    }
    sysSetInterval(40 / gameSpeedMultiple, MainGame$listenUIEvents$lambda_1(delta, this));
  };
  MainGame.$metadata$ = {
    kind: Kind_CLASS,
    simpleName: 'MainGame',
  ...
  Combat.prototype.prepareForNewCombat_0 = function () {
    ...
    while (tmp$_1.hasNext()) {
      var m_0 = tmp$_1.next();
      this.mWinMoney_0 = this.mWinMoney_0 + (m_0.money | 0) * window.winMoneyMultiple;
      this.mWinExp_0 = this.mWinExp_0 + (m_0.exp | 0) * window.winExpMultiple;
    }
    console.log("=====================================" + this.mWinMoney_0 + " " + this.mWinExp_0);
  ...
  function DatLib$Companion$instance$lambda() {
    var libName = 'FMJ';
    if (window.choiceLibName) {
      libName = window.choiceLibName;
    }
    return new DatLib(File$Companion_getInstance().contentsOf_61zpoe$(libName+".LIB"));
  }
  ...
  ResMagicChain.prototype.getAllLearntMagics = function () {
    ...
    var a = this.learnNum;
    if (a <= 0) {
       a = this.mMagics_0.size;
    }
    var b = this.magicSum_0;
    let magicList = slice(tmp$, until(0, Math_0.min(a, b)));
    if (window.magicReverse == true) {
      magicList.array_hd7ov6$_0 = magicList.array_hd7ov6$_0.reverse(); 
    }
    return magicList;
  };
